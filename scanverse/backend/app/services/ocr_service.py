...
def extract_text(
    ...
    *,
    ...
    """
    Run OCR on an image — fast single-pass strategy.

    Uses a single Tesseract pass with PSM 6 (uniform block, good for
    newspapers and most documents). Preprocessing is lightweight to
    avoid timeouts on free-tier deployments.
    ...
    # Lightweight preprocessing: just grayscale + contrast + threshold
    if preprocess:
        gray = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2GRAY)
        # Mild CLAHE contrast boost (no slow bilateral/unsharp)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        gray = clahe.apply(gray)
        # Otsu binarization for clean text
        _, gray = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    else:
        gray = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2GRAY)

    # Single Tesseract pass — PSM 6 works well for most documents
    try:
        result = _run_tesseract(gray, lang_config, 6)
        data = result["data"]
    except Exception:
        # Fallback to PSM 3 if PSM 6 fails
        try:
            result = _run_tesseract(gray, lang_config, 3)
            data = result["data"]
        except Exception:
            # Absolute fallback
            data = pytesseract.image_to_data(
                gray, lang=lang_config,
                config="--psm 3 --oem 1",
                output_type=pytesseract.Output.DICT,
            )
